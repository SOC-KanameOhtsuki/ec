...
/**
 * Class ProductTransmittalPdfService.
 * Do export pdf function.
 */
class ProductTransmittalPdfService extends AbstractFPDIService
{
    // ====================================
    // 定数宣言
    // ====================================
    ...
    public function makePdf(array $orders)
    {
        ...
        foreach ($orders as $order) {
            // PDFにページを追加する
            $templateFilePath = __DIR__.'/../Resource/pdf/product_transmittal1.pdf';
            $this->setSourceFile($templateFilePath);
            $this->addPdfPage();
            // 作成日
            $this->lfText(147.0, 38.4, date('Y'), 13, 'B');
            $this->lfText(162.5, 38.4, date('n'), 13, 'B');
            $this->lfText(169.7, 38.4, date('j'), 13, 'B');
            // 会員名
            $this->lfText(16.3, 66.8, $order->getName01() . $order->getName02(), 15, 'B');
            // PDFにページを追加する
            $templateFilePath = __DIR__.'/../Resource/pdf/product_transmittal2.pdf';
            $this->setSourceFile($templateFilePath);
            $this->addPdfPage();
        }
