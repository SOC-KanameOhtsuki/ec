...
/**
 * Class FaxAcceptFormPdfService.
 * Do export pdf function.
 */
class FaxAcceptFormPdfService extends AbstractFPDIService
{
    // ====================================
    // 定数宣言
    // ====================================
     ...
     * @return bool
     */
    public function makePdf(array $orders)
    {
        // データが空であれば終了
        if (count($orders) < 1) {
            return false;
        ...
        foreach ($orders as $order) {
            // PDFにページを追加する
            $templateFilePath = __DIR__.'/../Resource/pdf/acceptance_form_fax1.pdf';
            $this->setSourceFile($templateFilePath);
            $this->addPdfPage();
            // 作成日
            $this->lfText(163.5, 17.5, date('Y'), 13, 'B');
            $this->lfText(178.5, 17.5, date('n'), 13, 'B');
            $this->lfText(187.0, 17.5, date('j'), 13, 'B');
            // Fax番号
            if (!is_null($order->getFax01()) && !is_null($order->getFax02()) && !is_null($order->getFax03())) {
                $this->lfText(52.3, 25.5, $order->getFax01() . '-' . $order->getFax02() . '-' . $order->getFax03(), 15, 'B');
            }
            // 会員名
            $this->lfText(15.0, 40.6, $order->getName01() . $order->getName02(), 15, 'B');
            $this->lfText(36.6, 89.5, $order->getName01() . $order->getName02(), 15, 'B');
            // 受付番号
            $this->lfText(38.9, 104.5, $order->getId(), 15, '');
            // PDFにページを追加する
            $templateFilePath = __DIR__.'/../Resource/pdf/acceptance_form_fax2.pdf';
            $this->setSourceFile($templateFilePath);
            $this->addPdfPage();
        }
